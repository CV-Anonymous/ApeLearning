...
from .our_loss import SelfLoss

def make_loss(cfg, num_classes):    # modified by gu
    sampler = cfg.DATALOADER.SAMPLER
    if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
        triplet = TripletLoss(cfg.SOLVER.MARGIN)  # triplet loss
    elif cfg.MODEL.METRIC_LOSS_TYPE == 'ours':
        #ours = SelfLoss(cfg.OURS.ALPHA, cfg.OURS.BETA, num_classes, cfg.OURS.EMBEDDING_SIZE)
        ours = SelfLoss(cfg.OURS.ALPHA, cfg.OURS.BETA, num_classes, cfg.OURS.EMBEDDING_SIZE)
        triplet = TripletLoss(cfg.SOLVER.MARGIN)  # triplet loss
    elif cfg.MODEL.METRIC_LOSS_TYPE == 'ours_triplet':
        ours = SelfLoss(cfg.OURS.ALPHA, cfg.OURS.BETA, num_classes, cfg.OURS.EMBEDDING_SIZE)
        triplet = TripletLoss(cfg.SOLVER.MARGIN)  # triplet loss
    ...
    elif cfg.DATALOADER.SAMPLER == 'ours':
        def loss_func(score, feat, target):
            loss, acc, proxypos, proxyneg, possim, negsim = ours(feat, target)
            if cfg.MODEL.IF_TRIPLET == 'yes':
                return (1.0 / 1.0) * loss + (1.0 / 1.0) * triplet(score, target)[0], acc, proxypos, proxyneg, possim, negsim
            else:
                return loss, acc, proxypos, proxyneg, possim, negsim
    elif cfg.DATALOADER.SAMPLER == 'ours_triplet':
        def loss_func(score, feat, target):
            loss, acc, proxypos, proxyneg, possim, negsim = ours(feat, target)
            return loss + 20 * triplet(score, target)[0], acc, proxypos, proxyneg, possim, negsim
    ...
    if cfg.MODEL.METRIC_LOSS_TYPE == 'ours' and cfg.DATALOADER.SAMPLER == 'ours':
        return ours, loss_func
    elif cfg.DATALOADER.SAMPLER == 'ours_triplet':
        return ours, loss_func
    else:
        return loss_func
